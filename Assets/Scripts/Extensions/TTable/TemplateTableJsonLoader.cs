        ...
        where TValue : class, new()
    {
        ...
        private static Func<TValue, TKey> GetIdGetter()
        {
            ...
            if (idField != null)
            {
                return value =>
                {
                    object fieldValue = idField.GetValue(value);
                    if (fieldValue is TKey key)
                    {
                        return key;
                    }
                    return (TKey)Convert.ChangeType(fieldValue, typeof(TKey));
                };
            }
            ...
            if (idProperty != null)
            {
                return value =>
                {
                    object propertyValue = idProperty.GetValue(value, null);
                    if (propertyValue is TKey key)
                    {
                        return key;
                    }
                    return (TKey)Convert.ChangeType(propertyValue, typeof(TKey));
                };
            }
